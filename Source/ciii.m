...
for i = 1:length(melhorRedesPaths)
    ...
    nomeRedeTreinada = ['melhor_rede_totaliii_' num2str(i) '.mat'];
    ...
    % ---------- Testes por pasta ----------
    for pt = 1:length(pastas)
        pastaTeste = pastas{pt};
        disp(['Testando rede ', num2str(i), ' com imagens da pasta "', pastaTeste, '"...']);
        [dataTeste, labelsTestes, targetsTeste] = carregarImagensFcn({pastaTeste}, classes, imgSize);
        ...
        for r = 1:numRepeticoes
            outTeste = net(dataTeste);
            [~, predTeste] = max(outTeste);
            [~, realTeste] = max(targetsTeste);
        ...
        resultados = [resultados; {i, accGlobal, pastaTeste, accTeste, evalc('disp(confMat)'), classAcc, bestClass, bestAcc, worstClass, worstAcc}];
    end
end

% ---------- Exportar para Excel ----------
disp('Salvando resultados no arquivo "resultados_c_iii.xlsx"...');
...
writetable(T_final, 'resultados_c_iii.xlsx', 'WriteMode', 'overwrite');
